...
class Decoder(nn.Module):
    def __init__(
        ...
        # nn.Linear(in_features,out_features,bias=True) 做线性变换
        ...
        self.pointer = Attention(
            hidden_dim, use_tanh=use_tanh, C=tanh_exploration, use_cuda=self.use_cuda
        )
        ...
        if self.decode_type == "stochastic":
            for i in steps:
                ...
                decoder_input, idxs = self.decode_stochastic(
                    probs, embedded_inputs, selections
                )
            ...
            for i in steps:
                ...
                probs = (
                    probs.view(self.beam_size, batch_size, -1)
                    .transpose(0, 1)
                    .contiguous()
                )
    ...
    def decode_beam(self, probs, embedded_inputs, beam, batch_size, n_best, step):
        ...
        all_idxs = Variable(
            torch.LongTensor([[x for x in hyp] for hyp in all_hyp]).squeeze()
        )
        ...
        if idxs.dim() > 1:
            x = embedded_inputs[
                idxs.transpose(0, 1).contiguous().data,
...
class CriticNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.process_block = Attention(
            hidden_dim, use_tanh=use_tanh, C=tanh_exploration, use_cuda=use_cuda
        )
        self.sm = nn.Softmax()
...
class NeuralCombOptRL(nn.Module):
    ...
    def __init__(
        ...
        embedding_dim=128,
        hidden_dim=128,
        max_decoding_len=args.user_num,
        terminating_symbol="<0>",
        n_glimpses=2,
        n_process_block_iters=1,
        tanh_exploration=10,
        use_tanh=True,
        beam_size=1,
        objective_fn=None,
    ...
    def forward(self, inputs):
        ...
        # action_id为每次的120维动作；action_idxs为二维张量，一维代表节点个数的顺序，一维代表120个节点在该次序各自输出的序号
...
# net=NeuralCombOptRL()
# x=torch.tensor([[[2.4,21.42,3.12,4.25]],[[1,2,3,4]]])
# print(net(x))
# R, probs, actions, action_idxs=net(x)
