...
def show_order_influence():
    ...
    for user_number in user_number_list:
        for _ in range(1):
            ...
            throughput_his = duibi_exhaustive_search(
                users, 0, 1e-8, need_throughput_his=True
            )
            throughput_his = np.asarray(throughput_his)
            print(throughput_his)
            print(
                f"max={throughput_his.max()},min={throughput_his.min()},mean={throughput_his.mean()}"
            )
        ...
        for method_name, method in methods.items():
            ...
            if method_name == "duibi_exhaustive_search":
                ...
                if no_error:
                    continue
                try:
                    _, t_throuhput, t_ten_throughput, t_ten_decode_order = method(
                        users=users, noise=args.noise, alpha=args.alpha
                    )
